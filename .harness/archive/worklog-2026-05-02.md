...
## Session 2026-05-02 09:41 — eugene 5/2 06:00 sync 실패 진단 + timeout/allSettled fix + today.period 파싱

### 작업 요약
- **신고**: eugene 5/2 06:00 SGT 수집 안 됨. Mac은 켜져 있었음
- **진단** (submit.log + Supabase MCP):
  - `[2026-05-01T22:01:06.333Z] ERROR: codeburn/ccusage failed — codeburn timeout (period=today)`
  - codeburn `--period today` 60초 timeout 초과 → `Promise.all` reject → ingest 스킵
  - 정상 시 14~15초인데 가끔 60초+ 변동성
- **DB 상태 확인**: 본인은 5/2 00:00 SGT까지 정상, 06:00 실패. 다른 4명 자가 치유 부분 진행 (oreo OK, Junghwan 4/30 이후 미동작, Youngjin x2는 ccusage/codeburn 라벨 일치 안 됨 — 옛 submit.mjs로 추정)
- **3개 fix** (commit `fb5815e`):
  - codeburn timeout 60s → 180s
  - `Promise.all` → `Promise.allSettled`: today만 실패해도 나머지 4 period + ccusage는 ingest로 전송
  - `deriveUserTodayFromBody`에 `today.period` 라벨 정규식 파싱 추가 — `daily` 비어있어도 `"Today (2026-05-02)"`에서 날짜 추출
- **추가 timeout 확장**:
  - codeburn 180s → 600s (commit `d29ed62`)
  - ccusage 60s → 600s (commit `a95dae2`)
  - 10분이면 모든 정상 codeburn 케이스 커버, 진짜 hung만 차단

### 다음 액션
- 본인 머신 repair → 새 submit.mjs 받기 (timeout 600s + allSettled 적용)
- 다른 4명 팀원도 repair 필요 (TZ env 주입 + timeout 적용)
- 다음 launchd sync에서 더 이상 timeout 안 나는지 검증

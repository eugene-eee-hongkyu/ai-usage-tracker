...
function getApiKeyViaLocalServer() {
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      if (apiKey) {
        res.end("<html><head><meta charset='utf-8'></head><body style='font-family:sans-serif;padding:2em'>" + "<h2>✅ 인증 완료</h2><p>이 창을 닫아도 됩니다.</p></body></html>");
...
function runBackfill(apiKey) {
  ...
  if (!fs.existsSync(syncScript)) {
    const srcSync = path.join(__dirname2, "sync.mjs");
    if (fs.existsSync(srcSync))
      fs.copyFileSync(srcSync, syncScript);
  }
  if (!fs.existsSync(syncScript))
    return;
...
function spawnCcusage(since) {
  return new Promise((resolve, reject) => {
    const chunks = [];
    const proc = spawn2("npx", ["ccusage", "daily", "--json", "--since", since], {
...
program.name("ai-usage-tracker").description("Primus Labs Claude Code usage tracker").version("0.1.0");

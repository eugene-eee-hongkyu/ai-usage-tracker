...
|---|---|---|---|
| 2026-05-30 | 24bf0dd (playbook 작성 직후) | Anchor 1 (본인) /dashboard 통과 + 정정 사항 반영 완료 | Hero 5 카드 / Period 토글 / nav / Daily Activity 차트 / "Show details" 누락 보완. Daily Unit Cost = period 따라 일수 변하는 게 정상 동작 (잘못된 memory 삭제). ranking "streak" 라벨 → "연속 활성일" 정정 |
| 2026-05-30 | 7c30cc6 (1차 정정 직후) | Anchor 2 (영진님) + Anchor 3 (oreo) 부분 검증 → 대규모 정정 | (1) /team/[userId]/dashboard → /team/[userId] redirect, 단순화된 프로필 view (Hero 4 카드 + heatmap 4 weeks + Top projects 10), dashboard-view 풀 화면 아님. (2) /team/[userId] 의 "No data yet" 분기 발견 (DB 데이터 있어도 최근 활동 기준). (3) /platform-admin/all-users 카드 클릭 = view-as 활성 + /admin/members landing (자동 /dashboard X). (4) device 분리 = /platform-admin/all-users 의 카드 2 개 분리 (영진님 Mac + Windows 별 카드). (5) Codex modal 자동 trigger = 다른 사용자 view-as 시 안 뜸. (6) oreo Hero 938.3M/$634.22/97.6%/69%/Active 6 days, efficiency "No data/No activity" 노출 — Codex 경고 케이스 진앙 추정. 다음 라운드: oreo codex 토글 후 정확한 경고 표현 검증 + 영진님 view-as → 풀 dashboard 의 device 별 line 분리 동작 검증 |

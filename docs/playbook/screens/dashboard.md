...
| 특정 멤버의 이번달 / 30일 / 전체 차트가 0 (8일은 정상) | CLI partial ingest — `codeburn --provider X --period {month,30days,all}` 호출 실패. 2026-05-30 v0.3.2 fix 세트: **A** (서버 UI fallback chain) + **B** (서버 `raw_json` jsonb merge — partial 와도 기존 키 보존) + **C'** (CLI 가 partial 라도 보내고 `claudeFailPeriods` / `codexFailPeriods` telemetry 같이 전송) + **E** (CLI `telemetry-buffer.jsonl` — 모든 실패 누적, 다음 ingest 에 함께 전송, 서버 ack 후 clear) + **F1** (CLI provider 간 직렬 — 동시 spawn 14 → 7, 자원 경합 완화). 서버 진단 정보 = `api_tokens.metadata.lastIngestTelemetry / lastClaudeFailPeriods / lastCodexFailPeriods / lastIngestAt` |

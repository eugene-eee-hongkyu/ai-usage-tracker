...
Write-Host ""
Write-Host "AI Usage Tracker" -ForegroundColor Cyan
Write-Host ""
...
if ($needsInstall) {
  if ($null -eq $nodeMajor) {
    ...
    Write-Host "Node $nodeMajor → Node 22 (LTS) 전환 중..." -ForegroundColor Yellow
  }

  $wingetCmd = Get-Command winget -ErrorAction SilentlyContinue
  if (-not $wingetCmd) {
    Write-Host ""
    Write-Host "winget 이 없어 Node 자동 설치가 불가합니다." -ForegroundColor Red
    Write-Host "Node.js 22 LTS 를 직접 설치 후 다시 실행해주세요:"
  ...
  winget install OpenJS.NodeJS.LTS --silent --accept-source-agreements --accept-package-agreements 2>$null | Out-Null
  winget upgrade OpenJS.NodeJS.LTS --silent --accept-source-agreements --accept-package-agreements 2>$null | Out-Null
  ...
  if ($null -eq $nodeMajor -or $nodeMajor -lt 22) {
    Write-Host ""
    Write-Host "Node 22 자동 설치 실패. 직접 설치해주세요:" -ForegroundColor Red
    Write-Host "   https://nodejs.org/ko/download (LTS 선택)"
    Write-Host "   설치 후 새 PowerShell 창에서:"
    Write-Host "     irm https://aiusage.z21labs.world/install.ps1 | iex"
    exit 1
  }
  Write-Host "   완료 ($(node -v))" -ForegroundColor Green
} else {
...
if ((Test-Path $apiKeyPath) -or (Test-Path $legacyKeyPath)) {
  Write-Host "🔄 이미 설치되어 있어 업데이트합니다..." -ForegroundColor Cyan
  Write-Host ""
  $env:AIUSAGE_FROM_INSTALL_SH = "1"
  npx --yes --ignore-cache github:eugene-eee-hongkyu/ai-usage-tracker repair
} else {
  Write-Host "🚀 처음 설치합니다 — 잠시 후 브라우저에서 로그인 화면이 열립니다." -ForegroundColor Cyan
  Write-Host ""
  $env:AIUSAGE_FROM_INSTALL_SH = "1"
